...
func init() {
	...
	defaultNodeHome = ynxconfig.MustGetDefaultNodeHome()
}
...
func NewApp(
	...
	app.AccountKeeper = authkeeper.NewAccountKeeper(
		...
		authtypes.ProtoBaseAccount, cosmosevmconfig.GetMaccPerms(),
	...
	app.BankKeeper = bankkeeper.NewBaseKeeper(
		...
		app.AccountKeeper,
		cosmosevmconfig.BlockedAddresses(),
		authAddr,
		logger,
	)
...
func (app *App) FinalizeBlock(req *abci.FinalizeBlockRequest) (res *abci.FinalizeBlockResponse, err error) {
...
func (app *App) DefaultGenesis() map[string]json.RawMessage {
	...
	// Bank metadata for NYXT.
	{
		var bankGenState banktypes.GenesisState
		app.appCodec.MustUnmarshalJSON(genesis[banktypes.ModuleName], &bankGenState)

		hasNYXT := false
		for _, md := range bankGenState.DenomMetadata {
			if md.Base == ynxconfig.BaseDenom {
				hasNYXT = true
				break
			}
		}
		...
		if !hasNYXT {
			bankGenState.DenomMetadata = append(bankGenState.DenomMetadata, banktypes.Metadata{
				Description: "NYXT is the native token of YNX.",
				DenomUnits: []*banktypes.DenomUnit{
					{Denom: ynxconfig.BaseDenom, Exponent: 0},
					{Denom: ynxconfig.DisplayDenom, Exponent: uint32(ynxconfig.BaseDenomUnit)},
				},
				Base:    ynxconfig.BaseDenom,
				Display: ynxconfig.DisplayDenom,
				Name:    "NYXT",
				Symbol:  "NYXT",
			})
		}
	...
	// Set bond denom to match the EVM gas denom.
	{
		var stakingGenState stakingtypes.GenesisState
		app.appCodec.MustUnmarshalJSON(genesis[stakingtypes.ModuleName], &stakingGenState)
		stakingGenState.Params.BondDenom = ynxconfig.BaseDenom
		genesis[stakingtypes.ModuleName] = app.appCodec.MustMarshalJSON(&stakingGenState)
	}

	// Align governance defaults with v0 spec.
	{
		var govGenState govv1.GenesisState
		app.appCodec.MustUnmarshalJSON(genesis[govtypes.ModuleName], &govGenState)

		params := govv1.DefaultParams()
		if govGenState.Params != nil {
			params = *govGenState.Params
		}

		depositAmount := sdkmath.NewIntWithDecimal(100_000, ynxconfig.BaseDenomUnit)
		params.MinDeposit = sdk.NewCoins(sdk.NewCoin(ynxconfig.BaseDenom, depositAmount))
		params.ExpeditedMinDeposit = sdk.NewCoins(sdk.NewCoin(ynxconfig.BaseDenom, depositAmount.MulRaw(2)))

		votingPeriod := 7 * 24 * time.Hour
		params.VotingPeriod = &votingPeriod
		params.Quorum = "0.1"
		params.Threshold = "0.5"
		params.VetoThreshold = "0.334"
