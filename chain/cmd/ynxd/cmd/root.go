...
// NewRootCmd creates a new root command for ynxd. It is called once in the
// main function.
func NewRootCmd() *cobra.Command {
	...
	initClientCtx := client.Context{}.
		...
		WithHomeDir(ynxconfig.MustGetDefaultNodeHome()).
	...
	rootCmd := &cobra.Command{
		Use:   "ynxd",
		Short: "YNX node (Cosmos SDK + Cosmos EVM)",
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			customAppTemplate, customAppConfig := cosmosevmconfig.InitAppConfig(
				ynxconfig.BaseDenom,
				ynxconfig.DefaultEVMChainID,
			)
...
func initRootCmd(rootCmd *cobra.Command, evmApp *ynx.App) {
	cfg := sdk.GetConfig()
	cfg.Seal()
...
func appExport(
	...
	if height != -1 {
		exampleApp = ynx.NewApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

		if err := exampleApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		exampleApp = ynx.NewApp(logger, db, traceStore, true, appOpts, baseapp.SetChainID(chainID))
	}
...
func getChainIDFromOpts(appOpts servertypes.AppOptions) (chainID string, err error) {
	...
	if chainID == "" {
		...
		chainID, err = cosmosevmconfig.GetChainIDFromHome(homeDir)
		if err != nil {
			return "", err
		}
	}
