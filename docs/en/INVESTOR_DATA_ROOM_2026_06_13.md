# YNX Investor Data Room Index

Status: active  
Last updated: 2026-06-13  
Canonical language: English

## Purpose

This file is the investor and grant-reviewer entry point for YNX.

## 1. Executive Narrative

- `README.md` — project overview and live public endpoints
- `docs/en/FUNDRAISING_MEMO_2026_06_13.md` — core financing narrative
- `docs/en/GRANT_BRIEF_2026_06_13.md` — short-form grant version
- `docs/en/YNX_POSITIONING.md` — positioning and external wording

## 2. Live Product Evidence

- `docs/en/PUBLIC_TESTNET_READINESS_REPORT_2026_05_01.md`
- `docs/en/PUBLIC_TESTNET_STATUS_2026_05_02.md`
- `docs/en/PUBLIC_ASSET_STATUS.md`
- `docs/en/AI_WEB4_OFFICIAL_DEMO.md`
- `docs/en/TESTNET_LAUNCH_GRADE_RUNBOOK.md`

## 3. Product / Technical Scope

- `docs/en/YNX_v2_WEB4_SPEC.md`
- `docs/en/YNX_v2_WEB4_API.md`
- `docs/en/YNX_v2_AI_SETTLEMENT_API.md`
- `docs/en/V2_HIGH_ASSURANCE_CRYPTO_MODEL.md`
- `docs/en/YNX_ARES_HYBRID_CRYPTO_PROTOCOL.md`

## 4. Company / Compliance / Legal Boundary

- `docs/en/NON_CUSTODIAL_BUSINESS_AND_COMPLIANCE_BOUNDARY.md`
- `docs/en/COMPLIANCE_READINESS_PACKET_2026_06_13.md`
- `docs/en/TERMS_OF_USE_DRAFT_2026_06_13.md`
- `docs/en/PRIVACY_POLICY_DRAFT_2026_06_13.md`
- `docs/en/RISK_DISCLOSURES_DRAFT_2026_06_13.md`
- `docs/en/PROJECT_NON_TECHNICAL_LAUNCH_PACKET.md`
- `SECURITY.md`

## 5. Operational Readiness

- `docs/en/PRODUCTION_HARDENING_ROADMAP.md`
- `docs/en/MAINNET_AND_INDUSTRY_READINESS_GATES.md`
- `docs/en/V2_AUDIT_COMPLIANCE_EXECUTION_GUIDE.md`
- `docs/en/EXTERNAL_VALIDATOR_ONBOARDING_PACKET.md`

## 6. Internal Diligence Questions This Room Should Answer

An investor or grant reviewer should be able to answer:

- Is the project real and live?
- Is the product differentiated?
- Is the company relying on token speculation?
- Is the legal/commercial boundary sane?
- What are the major remaining execution risks?
- What exactly would new funding accelerate?

## 7. Current High-Level Conclusion

YNX appears strongest as:

- a non-custodial infrastructure and AI-execution company;
- a live public-testnet protocol stack with verifiable technical evidence;
- an early-stage project suitable for grants, pre-seed, or strategic infra
  funding.

YNX appears weakest when framed as:

- a near-term regulated exchange;
- a custody-heavy bridge business;
- a BNB-dependent ecosystem pitch;
- a token-price thesis without operating revenue.
