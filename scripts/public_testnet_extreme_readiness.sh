...
BRIDGE_READY_URL="${YNX_BRIDGE_READY_URL:-https://bridge.ynxweb4.com/ready}"
BRIDGE_ROUTES_URL="${YNX_BRIDGE_ROUTES_URL:-https://bridge.ynxweb4.com/bridge/routes}"
BRIDGE_ROUTE_CHECKS_URL="${YNX_BRIDGE_ROUTE_CHECKS_URL:-https://bridge.ynxweb4.com/bridge/route-checks}"
...
fetch bridge_ready "$BRIDGE_READY_URL"
fetch bridge_routes "$BRIDGE_ROUTES_URL"
fetch bridge_route_checks "$BRIDGE_ROUTE_CHECKS_URL"
...
bridge_ready="$(jq -r '.ok // false' "${OUTPUT_DIR}/responses/bridge_ready.json")"
bridge_route_count="$(jq -r '[.items[]?] | length' "${OUTPUT_DIR}/responses/bridge_routes.json")"
bridge_route_failures="$(jq -r '[.items[]? | select(.ok != true)] | length' "${OUTPUT_DIR}/responses/bridge_route_checks.json")"
...
[[ "$bridge_ready" == "true" ]] && record PASS "bridge_ready" "true" || record FAIL "bridge_ready" "$bridge_ready"
[[ "$bridge_route_count" == "5" ]] && record PASS "bridge_routes" "routes=${bridge_route_count}" || record FAIL "bridge_routes" "routes=${bridge_route_count}, expected=5"
[[ "$bridge_route_failures" == "0" ]] && record PASS "bridge_route_checks" "failures=0" || record FAIL "bridge_route_checks" "failures=${bridge_route_failures}"
