...
const required = [
  ["pull request gate for final branch", /pull_request:\s*\n\s+branches:\s*\n\s+-\s+codex\/final-security-platform\b/],
...
function main() {
  ...
  if (failures.length > 0) {
    ...
    process.stdout.write("PASS required workflow runs for every final-branch change and remains validation-only\n");
  }
}
